...
An example is seen in Figure \ref{fig:simwrapper-network-links}. User testing shows that a great deal more could still be done to meet user expectations. Data filters and configurable hovers are two highly-requested enhancements, as well as a first-class ``difference mode''.  \kai{There could be a bit more text saying something along the lines that in 7.4 width corresponds to capacity (or flow), and color to co2 difference.  Or whatever it is.}
