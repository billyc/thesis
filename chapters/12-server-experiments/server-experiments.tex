...
\small\emph{accessibilities:ke\_kibera\_osm\_live\_50\_drinking\_water\_walk\_0.5\_3.5}.\normalsize
...
YAML itself is a simple text format which uses indentation and punctuation to define and group pairs of \emph{key:value} configuration data. For example, \autoref{fig:geoserver-yaml} shows a snippet of a YAML configuration for drinking water data in Kibera, Kenya.
...
The size of the emission output files depends on the size of the grid cells, the number of time bins, and the geographic extent of the overall area being modeled. For the Berlin experiment, the use of one-hour time bins and 100 meter cells resulted in pollutant data files of approximately 150 megabytes per pollutant. Being text files, they compress by around 60-75\% -- but even so, these are rather large files to try to load into a local web browser.
...
The background task of calculating Voronoi cells at a specified zoom level is slow, taking many seconds or tens of seconds to update when the map zoom level changes. Worse, the resulting depiction has ``jagged'' display artifacts that are not representative of the underlying data and are not expected by analysts. Between the slowness of loading the data, the poor performance of the resulting visualization, and the compromises involved in viewing a limited level of detail at some zoom levels, this visualization experiment was not very useful for analysts at VSP.
